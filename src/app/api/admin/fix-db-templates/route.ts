...
export async function GET(req: NextRequest) {
  try {
    ...
    // 2. Kiểm tra xem template danh tính đã tồn tại chưa trước khi insert
    const identityKeyword = "ai là gì";
    const { data: existing } = await adminSupabase
      .from('chat_templates')
      .select('id')
      .ilike('keywords', `%${identityKeyword}%`)
      .limit(1);
    ...
    return NextResponse.json({ 
        success: true, 
        message: `Đã xóa ${idsToDelete.length} mẫu câu bị lỗi chính tả/ngáo ngơ. Template danh tính AI: ${existing && existing.length > 0 ? 'Tồn tại rồi, bỏ qua.' : 'Đã tạo mới.'}`,
